...
<div class="container mt-4">
    <h1>Создать урок для модуля "{{ $module->title }}" (курс "{{ $course->title }}")</h1>
    ...
    <form method="POST" action="{{ route('storeLesson', ['course' => $course, 'module' => $module]) }}">
        @csrf
        <div class="row">
            <div class="col-md-6">
                <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control @error('order') is-invalid @enderror" id="order" name="order" value="{{ old('order', $module->lessons->count() + 1) }}" min="1" required>
                    @error('order')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3 form-check">
                    <input type="checkbox" class="form-check-input @error('is_free_preview') is-invalid @enderror" id="is_free_preview" name="is_free_preview" value="1" {{ old('is_free_preview') ? 'checked' : '' }}>
