            ...
            <div class="flex flex-col gap-6">
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title', $module->title) }}" required class="w-full p-3 rounded-xl bg-gray-900 text-white border border-gray-600 outline-none">
                    </div>

                    <div>
                        <label for="order" class="block text-gray-300 mb-1">Порядок модуля</label>
                        <input type="number" name="order" id="order" value="{{ old('order', $module->order) }}" min="1" required class="w-full p-3 rounded-xl bg-gray-900 text-white border border-gray-600 outline-none">
                    </div>
                </div>
                <div class="space-y-4">
                    <div>
                        <label for="description" class="block text-gray-300 mb-1">Описание модуля</label>
                        <textarea name="description" id="description" rows="5" class="w-full p-3 rounded-xl bg-gray-900 text-white border border-gray-600 outline-none">{{ old('description', $module->description) }}</textarea>
                    </div>
                </div>
            </div>
